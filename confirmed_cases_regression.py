...
popt, pcov = curve_fit(gompertz_model, xdata, ydata, maxfev=5000)
popt2, pcov2 = curve_fit(logistic_model, xdata, ydata, maxfev=5000)
# xdata_prediction = [i for i in range(int(2*popt[1]))]  # logistic
xdata_prediction = [i for i in range(250)]  # gompertz

plt.plot(gompertz_model(xdata_prediction, *popt), 'r--')
plt.plot(logistic_model(xdata_prediction, *popt2), 'g--')
plt.plot(ydata, 'b')
